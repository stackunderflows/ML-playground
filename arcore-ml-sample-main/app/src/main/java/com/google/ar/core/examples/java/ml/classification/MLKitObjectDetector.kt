...
class MLKitObjectDetector(context: Activity) : ObjectDetector(context) {
  // To use a custom model, follow steps on https://developers.google.com/ml-kit/vision/object-detection/custom-models/android.
   val model = LocalModel.Builder().setAssetFilePath("inception.tflite").build()
//   val model = LocalModel.Builder().setAssetFilePath("mike-yolo.tflite").build()
  ...
  override suspend fun analyze(image: Image, imageRotation: Int): List<DetectedObjectResult> {
    ...
    return mlKitDetectedObjects.mapNotNull { obj ->
      ...
      val coords: Pair<Int, Int> = obj.boundingBox.exactCenterX().toInt() to obj.boundingBox.exactCenterY().toInt()
      val rotatedCoordinates = coords.rotateCoordinates(rotatedImage.width, rotatedImage.height, imageRotation)
      val rotatedTopLeft = (obj.boundingBox.left to obj.boundingBox.top).rotateCoordinates(rotatedImage.width, rotatedImage.height, imageRotation)
      val rotatedBottomRight = (obj.boundingBox.right to obj.boundingBox.bottom).rotateCoordinates(rotatedImage.width, rotatedImage.height, imageRotation)

      DetectedObjectResult(bestLabel.confidence, bestLabel.text, rotatedCoordinates, Rect(rotatedTopLeft.first, rotatedTopLeft.second, rotatedBottomRight.first, rotatedBottomRight.second))
    }
  }
